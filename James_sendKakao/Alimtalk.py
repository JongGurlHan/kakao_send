...
bp = Blueprint('alimtalk', __name__)
...
def index_alimtalk():
    ...
    return render_template('New3_Alimtalk_sendForm.html', lists = wholeTemplateList)


@bp.route('/sendAlimtalk', methods = ['POST'])
def sendMessage1():
        
    token = genToken()
    sender = request.form['sendNum'] 
    render_template('NEW3_Alimtalk_sendForm.html', lists = wholeTemplateList)
